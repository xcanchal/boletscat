...
globalThis.fetch = async url => {
  ...
  const variable=parsed.searchParams.get('$where').match(/codi_variable='(\d+)'/)?.[1];
  const values={ '35':12,'32':16,'33':70,'30':2,'36':180 };
  return Response.json(Array.from({ length:61 },(_,index)=>{
    const date=new Date(Date.UTC(2026,8,5-index));
    const value=values[variable]??0;
    return {
      codi_estacio:'UI',dia:date.toISOString().slice(0,10),value,
